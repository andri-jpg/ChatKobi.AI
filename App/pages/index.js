  ...
  function Expander({ title, content }) {
    ...
    return (
      <div className="expander">
        <div className="expander-header" onClick={toggleExpanded}>
          <h3>{title}</h3>
          <span className={`expander-icon ${expanded ? 'expanded' : ''}`}>&#9658;</span>
        </div>
        {expanded && <div className="expander-content">{content}</div>}
      </div>
    );
  }
      ...
      <main className={styles.main}>
        {termsAccepted ? (
          <><div className={styles.cloud}>
            <div ref={messageListRef} className={styles.messagelist}>
              {messages.map((message, index) => {
                return (
                  <div key={index} className={message.type === "userMessage" && loading && index === messages.length - 1 ? styles.usermessagewaiting : message.type === "apiMessage" ? styles.apimessage : styles.usermessage}>
                    {message.type === "apiMessage" ? <Image src="/bot.png" alt="AI" width="30" height="30" className={styles.boticon} priority={true} /> : <Image src="/usericon.png" alt="Me" width="30" height="30" className={styles.usericon} priority={true} />}
              ...
              <div className={styles.footer}>
                <p>Built by <a href="https://github.com/andri-jpg/ChatKobi.ai" target="_blank">Andri Lawrence</a>.</p>
              </div>
            </div></>
        ) : (
          <div className={styles.center}>
            ...
            <br/>
            <p>
            Chatbot ini menyediakan informasi kesehatan umum dan bukan pengganti konsultasi medis langsung dengan profesional kesehatan. <br/><br/>
            ...
            <br/><p>Informasi lebih lanjut <a href="https://github.com/andri-jpg/ChatKobi.AI#disclaimer" target="_blank" style={{ color: 'green' }}>Klik Disini</a>.
            </p><br/>
              <label>
              <input
              type="checkbox"
